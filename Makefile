# vgi-feed Makefile
#
# A VGI worker (Go) that fetches and parses RSS / Atom / JSON feeds into DuckDB
# rows. Targets:
#
#   make build       Build the worker + mock-server binaries
#   make test-unit   Run the pure-Go unit/integration tests (httptest)
...
WORKER_BIN  := vgi-feed-worker
MOCK_BIN    := mockserver
WORKER_CMD  := ./cmd/vgi-feed-worker
...
# Build both binaries, start the mock feed server on a free port (it prints
# PORT:<n>), point the worker's feed functions at it via VGI_FEED_TEST_URL (the
# base URL; the .test files append /rss, /atom, /json, /malformed), run the SQL
# suite, then stop the mock server.
test-sql: build
	@set -e; \
	...
	echo "mock feed server listening on 127.0.0.1:$$PORT (pid $$MOCK_PID)"; \
	set +e; \
	VGI_FEED_WORKER="$(WORKER_PATH)" \
	VGI_FEED_TEST_URL="http://127.0.0.1:$$PORT" \
