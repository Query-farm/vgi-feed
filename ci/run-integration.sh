...
#
# The feed worker fetches RSS/Atom/JSON over HTTP, so the suite needs a feed
# server: this script builds the repo's `mockserver`, starts it on a free port,
# and points the tests at it via VGI_FEED_TEST_URL (mirroring `make test-sql`).
#
# Required environment:
#   HAYBARN_UNITTEST   path to the haybarn-unittest binary
#   VGI_FEED_WORKER    worker LOCATION the .test files ATTACH (the built Go
...
# --- Start the mock feed server (the .test files fetch from it) -------------
...
# VGI_FEED_TEST_URL, and kill the server on exit — exactly like `make test-sql`.
...
echo "Running suite (worker: $VGI_FEED_WORKER) ..."
