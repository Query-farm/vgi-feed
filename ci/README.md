# CI: the vgi-feed worker integration suite
...
vgi-feed VGI worker through the **real DuckDB `vgi` extension** on every push /
PR.
...
| `TRANSPORT`  | Worker launch                                          | `VGI_FEED_WORKER` (ATTACH LOCATION) |
| ------------ | ------------------------------------------------------ | ----------------------------------- |
| `subprocess` | extension spawns the binary over stdio                 | `/abs/path/to/vgi-feed-worker`      |
| `http`       | `vgi-feed-worker --http` (prints `PORT:<n>`)           | `http://127.0.0.1:<port>`           |
| `unix`       | `vgi-feed-worker --unix <sock>` (prints `UNIX:<path>`) | `unix://<sock>`                     |
...
**The mock feed server runs for every transport.** The feed worker's table
functions (`feed_items`, `feed_info`) still fetch RSS/Atom/JSON over HTTP
regardless of how DuckDB talks to the worker, so the script always builds and
starts `mockserver`, exports `VGI_FEED_TEST_URL`, and trap-kills it (plus any
out-of-band worker process) on exit.

The **full** suite (`feed_items.test` and `feed_info.test`) runs over all three
transports — **no tests are gated**.
...
`feed_items` streams one row per feed item across multiple `Process` exchanges.
Over the **stateless** HTTP transport the worker holds no live state between
ticks — the framework round-trips the producer state through an opaque
continuation token (gob-encoding the user state after each tick, emitting at most
one data batch per response, then resuming from the token). The position
therefore **must live in the serialized state**: a bare post-`Emit` `Done bool`
observes the pre-`Emit` snapshot on resume, re-emits the same rows forever, and
pins the worker in an infinite loop (subprocess/unix keep the live state in
memory, so they were unaffected and hid the bug).

The fix is an explicit gob-encodable **cursor** in the state — `Cursor[T]{ Rows
[]T; Offset int }` (in `internal/feedworker/functions.go`). `Process` emits a
...
1. **Build the worker** — `go build -o vgi-feed-worker ./cmd/vgi-feed-worker`.
   The resulting binary is a self-contained stdio worker the extension can
   spawn; `VGI_FEED_WORKER` (an absolute path) is the ATTACH `LOCATION`.
...
4. **Run** — [`run-integration.sh`](run-integration.sh) builds + starts the
   repo's `mockserver` on a free port (exporting `VGI_FEED_TEST_URL`), stages
   the preprocessed tree, resolves `VGI_FEED_WORKER` for the selected transport,
   ...
   non-zero and fails the job. The mock server (and any out-of-band worker) is
   killed on exit.

## Run it locally

```bash
go build -o vgi-feed-worker ./cmd/vgi-feed-worker
# point HAYBARN_UNITTEST at a haybarn-unittest binary (or a local DuckDB
# `unittest` built with the vgi extension):
HAYBARN_UNITTEST=/path/to/haybarn-unittest \
VGI_FEED_WORKER="$PWD/vgi-feed-worker" \
...
starts the mock feed server, and points the worker at `$(CURDIR)/vgi-feed-worker`
